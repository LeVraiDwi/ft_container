    ...
    template <class FT>
        ...
        template<class Key, class T, class Compare = std::less<Key>, class Allocator = std::allocator<Node<T> > >
            class rb_tree
            {
                ...
                typedef typename allocator_type::reference  difference_type;
                ...
                public:
                explicit    rb_tree(const key_compare& cmp, const allocator_type& alloc = allocator_type()): _root(NULL), _size(0), _cmp(cmp), _alloc(alloc)
                {
                ...
                rb_tree(const rb_tree& other): _end(NULL), _root(NULL), _cmp(other._cmp), _alloc(other._alloc) {*this = other;}
