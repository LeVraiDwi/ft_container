...
namespace ft {
	...
	class map {
		public:
			...
			typedef rb_tree<Key, value_type, value_compare>	tree;
		...
		public:
			...
			map(const map& other): _tree(tree(value_compare(key_compare()))) { *this = other; }
			...
			const_iterator	find(const key_type& key) const { return (iterator(_tree.find(ft::make_pair(key, mapped_type())))); }
